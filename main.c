...
char parsedTitle[256] = { 0 };
char parsedDiscId[17] = { 0 };
char version[] = "UMDRescue v2.1.1";
...
//void dump() { //dump function returns exit int, change to int
void dump() {
	pspDebugScreenClear();
	...
	// Determine location to write iso depending on gtype variable [V]ideo or [G]ame] (improve this)
	if (gtype[0] == 'G')
