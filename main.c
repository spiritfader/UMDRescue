...
char *umdreadbuffer, discid[11], title[17], isopath[30], gtype[3], dir, vdir;
int threadnumber, bytesread, umdsize, dumppercent, byteswritten, sec = 0;
SceUID umd, iso, fd, threadlist[66], st_thlist_first[66], st_thnum_first;
...
int umdkiller_thread(SceSize args, void *argp){
  while (!leave){
    sceKernelDelayThread(50*1000);         // wait 50 microseconds for psp to boot
    sceCtrlPeekBufferPositive(&pad, 1);    // poll for button input
    if ((pad.Buttons & PSP_CTRL_SELECT)){  // launch UMDKillerPRX when SQUARE button is pressed
      ...
      oe_mallocinit(); 
	    if(!(umdreadbuffer=oe_malloc(1024))){ 
	    	  pspDebugScreenClear(); // clear screen
          int count = 11;
          while (count > 0) {
      ...
      if (iso<0){ // if isopath contains invalid characters or MS:0 is unable to be reached, this will result in MS0 error
        ...
        while (count > 0) {
          pspDebugScreenSetXY(0, 0); printf("%66s", "UMDKillerPRX 2.0");    
          pspDebugScreenSetXY(7, 12); printf("Error: can't access ms0: (memory stick)");
          pspDebugScreenSetXY(7, 14); printf("Exiting in %d seconds...",count); count = count - 1;
          sceKernelDelayThread(1000*1000);
        }
        pspDebugScreenSetXY(0, 0); printf("%66s", "UMDKillerPRX 2.0");    
      ...
      bytesread=sceIoRead(umd, umdreadbuffer, 512);
      ...
      while (bytesread > 0) {
        ...
        if (!(sceIoWrite(iso, umdreadbuffer, 512))){
          ...
          while (count > 0) {
            pspDebugScreenSetXY(0, 0); printf("%66s", "UMDKillerPRX 2.0");
            pspDebugScreenSetXY(7, 12); printf("Not enough free space on memory stick");
            pspDebugScreenSetXY(7, 14); printf("Exiting in %d seconds...",count); count = count - 1;
            sceKernelDelayThread(1000*1000);
          }
          pspDebugScreenSetXY(0, 0); printf("%66s", "UMDKillerPRX 2.0");
        ...
        if (!(bytesread=sceIoRead(umd, umdreadbuffer, 512))) {
          ...
          while (count > 0) {
            pspDebugScreenSetXY(0, 0); printf("%66s", "UMDKillerPRX 2.0");
            pspDebugScreenSetXY(7, 12); printf("UMD read error!");
            pspDebugScreenSetXY(7, 14); printf("Exiting in %d seconds...",count); count = count - 1;
            sceKernelDelayThread(1000*1000);
          }
          pspDebugScreenSetXY(0, 0); printf("%66s", "UMDKillerPRX 2.0");
        ...
        pspDebugScreenSetXY(15, 11); printf("Writing to %s",isopath);
	      pspDebugScreenSetXY(16, 15); printf("Writing Sectors: %d/%d - %d%% ",byteswritten,umdsize,dumppercent);
        pspDebugScreenSetXY(16, 17); printf("Writing Bytes: %d/%d - %d%% ",byteswritten*2048,umdsize*2048,dumppercent);
      } 
      ...
      while (count > 0) {
        pspDebugScreenSetXY(0, 0); printf("%66s", "UMDKillerPRX 2.0");
        pspDebugScreenSetXY(7, 12); printf("Successfully wrote to %s",isopath); 
        pspDebugScreenSetXY(7, 14); printf("Exiting in %d seconds...",count); count = count - 1;
        sceKernelDelayThread(1000*1000);
      }
      pspDebugScreenSetXY(0, 0); printf("%66s", "UMDKillerPRX 2.0");
